  ...
  useEffect(() => {
    async function loadPost() {
      try {
        ...
        const post = Array.isArray(posts) ? posts.find((p: { slug: string }) => p.slug === slug) : null;

        if (!post) {
          setMessage({ text: 'Post no encontrado', type: 'error' });
          setLoading(false);
          return;
        }
        ...
        // Match category case-insensitively to the CATEGORIES list
        const matchedCat = CATEGORIES.find(
          c => c.toLowerCase() === (post.category || '').toLowerCase()
        );
        setCategory(matchedCat || 'General');

        // Strip the heading line from content if it was prepended during creation
        let postContent = post.content || '';
        if (post.heading) {
          const headingLine = `# ${post.heading}`;
          const trimmed = postContent.trimStart();
          if (trimmed.startsWith(headingLine)) {
            postContent = trimmed.slice(headingLine.length).replace(/^\n+/, '');
          }
        }
        setContent(postContent);
      } catch {
        setMessage({ text: 'Error al cargar el post', type: 'error' });
      }
      setLoading(false);
    }
    loadPost();
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-zinc-950 text-zinc-100 flex items-center justify-center">
        <div className="text-zinc-500">Cargando post...</div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-zinc-950 text-zinc-100">
      ...
      <header className="border-b border-zinc-800 py-3 sm:py-4 px-4 sm:px-6">
        <div className="max-w-6xl mx-auto flex justify-between items-center">
          <div className="flex items-center gap-3">
            ...
            <span className="text-sm text-zinc-500">Editando: <span className="text-zinc-300">{slug}</span></span>
          </div>
          <div className="flex gap-2 sm:gap-4 items-center">
            ...
            <button
              onClick={handleSave}
              disabled={saving}
              className="px-6 py-2 rounded-lg bg-emerald-600 text-white font-semibold hover:bg-emerald-500 transition text-sm disabled:opacity-50"
            >
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-6 sm:py-8 flex flex-col lg:flex-row gap-6 lg:gap-8">
          ...
          {preview ? (
            <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-6 min-h-[400px]">
              <div className="prose prose-invert max-w-none">
                ...
                <p className="text-sm text-zinc-500 mb-6">{originalDate} · {category}</p>
                {tags.length > 0 && (
        ...
        <aside className="w-full lg:w-72 space-y-6">
          {/* Date (read-only info) */}
          <div className="space-y-2">
            <label className="text-xs text-zinc-500 font-semibold uppercase tracking-wider">Fecha de publicación</label>
            <div className="text-sm text-zinc-300 bg-zinc-900 border border-zinc-800 rounded-lg p-2.5">
              {originalDate}
            </div>
          </div>
          ...
          <div className="border-t border-zinc-800 pt-4 space-y-3">
            <Link
              href={`/p/${slug}`}
              className="block w-full text-center px-4 py-2 rounded-lg border border-zinc-700 text-zinc-400 hover:bg-zinc-800 transition text-sm"
            >
              Ver post publicado
            </Link>
            <Link
